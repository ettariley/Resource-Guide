...
function EditFeaturedResources() {
  ...
  const resourceTextDocRef = doc(db, 'Featured-Texts', 'ResourcePage');

  const saveEdits = () => {
    updateDoc(resourceTextDocRef, {
      Text: editingText,
    }).then(() => {
      setText(editingText);
      setReadOnly(true);
  ...
  useEffect(() => {
    const featuredTextQuery = query(resourceTextDocRef);
  ...
  return (
    <Fade in={open}>
      <Container className="mt-5 pt-5">
        <h2>Edit Resources Page Featured Text</h2>
        <Row>
...
export default EditFeaturedResources;
