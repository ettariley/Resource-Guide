...
function EditEvent() {
  ...
  const [pastEventReadOnly, setPastEventReadOnly] = useState(false);
  const [startDateReadOnly, setStartDateReadOnly] = useState(false);
  ...
  const isPast = (startDate, endDate) => {
  ...
  const searchEvents = () => {
    ...
    const eventsQSnap = getDocs(eventsQuery).then((eventsQSnap) => {
      ...
      if (includePast) {
        ...
        setEventPreviews(
          eventsArray.filter((event) => event.end >= new Date())
        );
      }
  ...
  const selectEvent = (selected) => {
    ...
    if (isPast(selected.start, selected.end)) {
      setPastEventReadOnly(true);
    } else {
      setPastEventReadOnly(false);
    }

    if (isInProgress(selected.start, selected.end)) {
  ...
  const disableEventEdit = () => {
    if (pastEventReadOnly || !readOnly) {
      return true;
    } else {
      return false;
    }
  ...
  const handleDeleteEvent = () => {
    ...
    deleteDoc(deleteDocRef).then(() => {
      setEventPreviews(eventPreviews.filter((ep) => ep.id !== event.id));
      cancelEdits();
      eventSelected.current = false;
      setEvent({});
      ...
      <Row>
                  ...
                  <Row className="gx-3">
                    {eventPreviews.map((ep) => (
                      <Col>
                        <Button
                          variant="link"
                          className="p-0 preview-cards w-100"
                          onClick={() => selectEvent(ep)}
                        >
                          <Card>
                            <Card.Header>
                            ...
                            {isPast(ep.start, ep.end) ? (
                              <Alert
                                className="ms-2 mb-2 me-2"
                                variant="danger"
                              >
                                This event has ended & can't be edited.
                              </Alert>
                            ) : null}
                            {isInProgress(ep.start, ep.end) ? (
                              <Alert className="ms-2 mb-2 me-2">
        ...
        {searchLength.current === 0 ? (
          <>
            ...
            <Link to="/events" className="text-secondary fs-5">
              ...
              <Card.Body>
                <Form noValidate>
                  ...
                  {!readOnly ? (
                    <Row className="mb-3">
                      {startDateReadOnly ? (
                        <Col>
                          <p>Event Start:</p>
                          <p>
                            {event.start.toLocaleString(localizer, {
                              dateStyle: 'long',
                              timeStyle: 'short',
                            })}
                          </p>
                    ...
                    <p>
                      Time: {getFormattedEventDates(event.start, event.end)}
                    </p>
                  )}
                  ...
                  <Form.Group
                    className="mb-3"
                    controlId="eventForm.Description"
                  >
                    <Form.Label>
                  ...
                  <Form.Group
                    className="mb-3"
                    controlId="eventForm.Populations"
                  >
                      ...
                      <>
                        {tags.map((t) => (
                          <Form.Check
                            ...
                            id={t}
                            onChange={(e) =>
                              onEventChange('tags', e.target.value)
                            }
